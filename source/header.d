...
int set_header_context(Mustache.Context context, Request request, Output output) {
  ...
  if (user_id > 0) {
    context.useSection("logged_in");
		...
		auto rows = db.query!(int, int)(db.prepare_bind!(int)("SELECT is_admin, is_trusted FROM users WHERE user_id = ?", user_id));
